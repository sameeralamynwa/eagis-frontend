...
interface ISymptomCheckerContext {
  messages: ChatMessage[];
  sendMessage: (message: ChatMessage) => Promise<void>;
  loadingResponse: boolean;
  scrollDivRef: RefObject<HTMLDivElement | null>;
  scrollDivToBottom: () => void;
}

// @ts-ignore
const SymptomCheckerContext = createContext<ISymptomCheckerContext>();

export const SymptomCheckerContextProvider = (props: PropsWithChildren) => {
  ...
  const scrollDivToBottom = () => {
    setTimeout(() => {
      if (scrollDivRef.current) {
        scrollDivRef.current.scrollTop = 10000000;
      }
    }, 100);
  };
...
export const useSymptomCheckerContext = () => useContext(SymptomCheckerContext);
